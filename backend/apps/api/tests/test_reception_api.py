...
class ReceptionAPITests(TestCase):
    ...
    def test_system_status_authenticated(self):
        ...
        self.assertNotIn("last_webhook_at", data["messaging"])
        self.assertIn("conversation", data)
        self.assertEqual(data["conversation"]["metrics_scope"], "cluster")
        self.assertIn("channex", data["conversation"]["channels"])
        self.assertIn("whatsapp", data["conversation"]["channels"])
        self.assertIn("email", data["conversation"]["channels"])
        self.assertIn("last_webhook_at", data["conversation"]["channels"]["channex"])
        self.assertIn("last_poll_at", data["conversation"]["channels"]["channex"])
        self.assertIn("ingest_lag_seconds", data["conversation"]["channels"]["channex"])

...
async function proxyInvoiceDetails(path: string, init?: RequestInit): Promise<NextResponse> {
...
export async function GET(request: Request, { params }: Params) {
  ...
  return proxyInvoiceDetails(
    `/api/v1/public/invoice-details/${encodeURIComponent(token)}/${qs}`,
  );
}
...
export async function PATCH(request: Request, { params }: Params) {
  ...
  return proxyInvoiceDetails(`/api/v1/public/invoice-details/${encodeURIComponent(token)}/`, {
    method: "PATCH",
    body,
  });
}
